...
# Convert text to numerical blocks
...
# Convert numerical blocks back to text
def blocks_to_text(blocks):
    return ''.join([chr(int(round(num))) if 32 <= num <= 126 else '?' for block in blocks for num in block])
...
def generate_key_matrix(block_size):
    while True:
        ...
        det = int(Matrix(key_matrix).det()) % 256

        # Ensure determinant is nonzero and has a modular inverse under mod 256
        if det != 0 and gcd(det, 256) == 1:
            try:
                _ = Matrix(key_matrix).inv_mod(256)  # Verify inverse exists
                return key_matrix
            except:
                continue  # Retry with a new matrix
...
def hill_cipher_encrypt(blocks, key_matrix):
    ...
    for block in blocks:
        ...
        encrypted_block = (np.dot(key_matrix, block_vector) % 256).astype(np.uint8)
...
def hill_cipher_decrypt(blocks, key_matrix):
    ...
    for block in blocks:
        block_vector = np.array(block).reshape((len(block), 1))
        decrypted_block = (np.dot(key_matrix_inv, block_vector) % 256).astype(np.uint8)
...
def transposition_encrypt(blocks, transposition_key):
    seed(sum(ord(char) for char in transposition_key))  # Hash-like seed
